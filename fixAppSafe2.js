...
// 1. ScrapedEventsPanel Header
const panelHeaderOld = `function ScrapedEventsPanel({ pendingScrapes, onRefresh }: { pendingScrapes: any[], onRefresh: () => void }) {
...
const panelHeaderNew = `function ScrapedEventsPanel({ pendingScrapes, orgs, onRefresh }: { pendingScrapes: any[], orgs: any[], onRefresh: () => void }) {
  const [expandedPending, setExpandedPending] = useState<{ id: string, type: 'org' | 'contacts' } | null>(null);
...
content = content.replace(panelHeaderOld, panelHeaderNew);

// 2. Pass orgs in ScrapedEventsPanel
const oldCalls = `<PendingEventRow 
                key={scrape.id} 
                scrape={scrape} 
                onRefresh={onRefresh} 
...
const newCalls = `<PendingEventRow 
                key={scrape.id} 
                scrape={scrape} 
                orgs={orgs}
                onRefresh={onRefresh} 
...
// 3. Replace PendingEventRow
const startIndexRow = content.indexOf('function PendingEventRow({ scrape, onRefresh, onViewOrg');
const endIndexRow = content.indexOf('function ScrapedUrlsPanel(', startIndexRow);

if (startIndexRow !== -1 && endIndexRow !== -1) {
    const oldRow = content.substring(startIndexRow, endIndexRow);
    const newRow = `function PendingEventRow({ scrape, orgs, onRefresh, onViewOrg }: { scrape: any, orgs: any[], onRefresh: () => void, onViewOrg: () => void }) {
...
`;
    ...
    console.log("Successfully rebuilt App.tsx properly");
} else {
    console.error("Could not find PendingEventRow bounds! start: " + startIndexRow + " end: " + endIndexRow);
}

...
// 2. Pass orgs={orgs} to ScrapedEventsPanel inside Dashboard
content = content.replace(
...
// 3. Update ScrapedEventsPanel
const oldScrapedPanel = `function ScrapedEventsPanel({ pendingScrapes, onRefresh }: { pendingScrapes: any[], onRefresh: () => void }) {
...
const newScrapedPanel = `function ScrapedEventsPanel({ pendingScrapes, orgs, onRefresh }: { pendingScrapes: any[], orgs: any[], onRefresh: () => void }) {
  const [expandedPending, setExpandedPending] = useState<{ id: string, type: 'org' | 'contacts' } | null>(null);
...
content = content.replace(oldScrapedPanel, newScrapedPanel);

// 4. Update PendingEventRow
const startIndexRow = content.indexOf('function PendingEventRow({ scrape, onRefresh, onViewOrg');
const endIndexRow = content.indexOf('function EventsPanel(', startIndexRow);
...
`;
    ...
    console.error("Could not find PendingEventRow bounds!");
}

...
const regex = /function ScrapedEventsPanel\(\{ pendingScrapes, orgs, onRefresh \}: \{ pendingScrapes: any\[\], orgs: any\[\], onRefresh: \(\) => void \}\) \{[\s\S]*?(?=function PendingEventRow)/;

const replacement = `function ScrapedEventsPanel({ pendingScrapes, orgs, onRefresh }: { pendingScrapes: any[], orgs: any[], onRefresh: () => void }) {
...
if(content.match(regex)) {
    ...
    console.log("Successfully replaced ScrapedEventsPanel using regex.");
} else {
    console.log("Still failed to match ScrapedEventsPanel.");
}

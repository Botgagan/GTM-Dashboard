...
// Fix 1: Pass orgs in the map
content = content.replace(
    /key=\{scrape\.id\}\s*scrape=\{scrape\}\s*onRefresh=\{onRefresh\}/,
    'key={scrape.id} scrape={scrape} orgs={orgs} onRefresh={onRefresh}'
);

// Fix 2: Add handleLinkOrg inside PendingEventRow
const anchorApprove = `  const handleApprove = async () => {`;
const insertLinkMethod = `  const isLinked = !!scrape.linked_org_id;
...
content = content.replace(anchorApprove, insertLinkMethod);

// Fix 3: Replace the Status Cell in PendingEventRow
const oldStatusCellRegex = /<TableCell>\s*\{isResolved \? \(\s*<Badge[^>]*>\s*\{scrape\.status\}\s*<\/Badge>\s*\) : \(\s*<Badge[^>]*>\s*\{eventStatus\}\s*<\/Badge>\s*\)\}\s*<\/TableCell>/g;

const newStatusCell = `<TableCell>
        {isResolved ? (
...
// Fix 4: Replace the Actions Cell
const oldActionsCell = `<TableCell className="text-right align-middle">
...
const newActionsCell = `<TableCell className="text-right align-middle">
        <div className="flex items-center justify-end gap-2 relative">
          {!isResolved ? (
            <>
              ...
              {isLinking && (
                <div className="absolute right-0 top-10 w-[300px] z-50 bg-white border shadow-lg rounded-md p-2 text-left">
...
console.log("Re-applied UI fixes safely");

...
const regexStart = /function ScrapedEventsPanel\(\{ pendingScrapes, orgs, onRefresh \}: \{ pendingScrapes: any\[\], orgs: any\[\], onRefresh: \(\) => void \}\) \{\s*const \[expandedPending, setExpandedPending\] = useState<\{ id: string, type: 'org' \| 'contacts' \} \| null>\(null\);\s*const activeScrape = expandedPending \? pendingScrapes\.find\(s => s\.id === expandedPending\.id\) : null;\s*return \(\s*<>\s*<Table>/m;

const newStart = `function ScrapedEventsPanel({ pendingScrapes, orgs, onRefresh }: { pendingScrapes: any[], orgs: any[], onRefresh: () => void }) {
...
if (regexStart.test(content)) {
    content = content.replace(regexStart, newStart);
    fs.writeFileSync('frontend/src/App.tsx', content, 'utf-8');
    console.log("Successfully injected filteredScrapes via regex");
} else {
    console.log("Regex match failed! Let's find the exact lines.");
}
